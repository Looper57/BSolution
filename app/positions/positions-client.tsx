...
import { positionDetailLocale } from '@/lib/routes'
...
function PositionCard({ job, featured = false }: { job: Job; featured?: boolean }) {
  ...
  if (featured) {
    return (
      <Link href={localizedPath(positionDetailLocale(locale), `/positions/${job.slug}`)} className="block bg-navy p-8 lg:p-10 group hover:bg-navy-light transition-colors">
  ...
  return (
    <Link
      href={localizedPath(positionDetailLocale(locale), `/positions/${job.slug}`)}
